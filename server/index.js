...
const spotifyTokenRequest = async (body) => {
...
const spotifyGet = async (url, accessToken) => {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Spotify API failed: ${text}`);
  }

  return response.json();
};

const mapSpotifyTrack = (track, baseUrl = '') => {
  ...
  return {
    ...
    audioUrl: previewUrl ? `${baseUrl}/api/audio?url=${encodeURIComponent(previewUrl)}` : undefined,
    preview_url: previewUrl,
    spotifyId: track.id,
    youtubeVideoId: undefined,
  };
...
app.get('/api/spotify/auth-url', (req, res) => {
  const clientId = process.env.SPOTIFY_CLIENT_ID;
  if (!clientId) {
    return res.status(500).json({ error: 'SPOTIFY_CLIENT_ID is missing' });
  }
...
app.post('/api/spotify/token', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to exchange Spotify code', details: String(error) });
  }
});
...
app.get('/api/spotify/playlists', async (req, res) => {
  ...
  try {
    const data = await spotifyGet('https://api.spotify.com/v1/me/playlists?limit=50', accessToken);
    ...
    res.status(500).json({ error: 'Failed to load Spotify playlists', details: String(error) });
  }
});
...
app.get('/api/spotify/playlists/:playlistId/tracks', async (req, res) => {
  ...
  try {
    ...
    const data = await spotifyGet(
      `https://api.spotify.com/v1/playlists/${req.params.playlistId}/tracks?limit=100`,
      accessToken
    );
    ...
    const tracks = (data.items ?? [])
      .map((item) => item.track)
      .filter(Boolean)
      .map((track) => mapSpotifyTrack(track, baseUrl));

    res.json({ tracks });
  } catch (error) {
    res.status(500).json({ error: 'Failed to load Spotify playlist tracks', details: String(error) });
  }
});
